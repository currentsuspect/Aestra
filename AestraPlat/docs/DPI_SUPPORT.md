# DPI Support in AestraPlat

## Overview

AestraPlat now includes comprehensive DPI (Dots Per Inch) awareness support for Windows, enabling applications to render correctly on high-DPI displays with proper scaling.
...
- `AestraPlat/src/Win32/PlatformDPIWin32.h` - DPI helper class interface
- `AestraPlat/src/Win32/PlatformDPIWin32.cpp` - DPI implementation
- `AestraPlat/src/Tests/PlatformDPITest.cpp` - DPI test application

### Files Modified

- `AestraPlat/CMakeLists.txt` - Added DPI files to build
- `AestraPlat/src/Platform.cpp` - Initialize DPI awareness on startup
- `AestraPlat/include/AestraPlatform.h` - Added `getDPIScale()` and `setDPIChangeCallback()`
- `AestraPlat/src/Win32/PlatformWindowWin32.h` - Added DPI tracking
- `AestraPlat/src/Win32/PlatformWindowWin32.cpp` - Handle `WM_DPICHANGED` messages

### AestraUI Integration

- `AestraUI/Platform/NUIPlatformBridge.h` - Added `getDPIScale()` method
- `AestraUI/Platform/NUIPlatformBridge.cpp` - Forward DPI changes to renderer
