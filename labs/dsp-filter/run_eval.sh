...
do_build() {
  ...
  log "Building AestraFilterTest..."
  cmake --build "$BUILD_DIR" --target AestraFilterTest --parallel 2 || fail "Build failed"
}
...
if $REBUILD || [ ! -f "$BUILD_DIR/CMakeCache.txt" ]; then
  ...
  NEED=false
  for src in \
    "$REPO_ROOT/AestraAudio/include/DSP/Filter.h" \
    "$REPO_ROOT/AestraAudio/src/DSP/Filter.cpp" \
    "$REPO_ROOT/AestraAudio/include/DSP/FilterSIMD.h" \
    "$REPO_ROOT/Tests/AestraAudio/FilterTest.cpp" \
...
BIN=""
for candidate in \
  "$BUILD_DIR/Tests/AestraAudio/AestraFilterTest" \
  "$BUILD_DIR/bin/Tests/AestraAudio/AestraFilterTest" \
  "$BUILD_DIR/Tests/AestraFilterTest" \
  "$BUILD_DIR/bin/Tests/AestraFilterTest" \
  "$BUILD_DIR/AestraFilterTest"; do
  [ -x "$candidate" ] && BIN="$candidate" && break
done
if [ -z "$BIN" ]; then
  log "AestraFilterTest binary missing, building target..."
  cmake --build "$BUILD_DIR" --target AestraFilterTest --parallel 2 || fail "Build failed"
  for candidate in \
    "$BUILD_DIR/Tests/AestraAudio/AestraFilterTest" \
    "$BUILD_DIR/bin/Tests/AestraAudio/AestraFilterTest" \
...
[ -z "$BIN" ] && fail "Cannot find AestraFilterTest binary"

log "Binary: $BIN"

# Run
OUT_FILE="${RESULTS_DIR}/${RID}_filter.txt"
set +e
printf 'n\n' | "$BIN" > "$OUT_FILE" 2>&1
EXIT=$?
set -e
...
[ "$DECISION" = "reject" ] && REASON="AestraFilterTest exited with code $EXIT"
...
cat > "$SUMMARY_FILE" <<EOJSON
{
  ...
  "lanes": { "filter": { "binary": "$BIN", "exit_code": $EXIT, "output_file": "$OUT_FILE" } },
  "decision": { "status": "$DECISION", "reason": "$REASON", "hard_gate_failures": $([ "$EXIT" -ne 0 ] && echo "[\"AestraFilterTest exited with code $EXIT\"]" || echo "[]"), "advisory_flags": $ADVISORY_FLAGS }
}
EOJSON
