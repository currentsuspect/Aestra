...
do_build() {
  ...
  log "Building MathTests..."
  cmake --build "$BUILD_DIR" --target MathTests --parallel 2 || fail "Build failed"
  log "Building MathBenchmark..."
  cmake --build "$BUILD_DIR" --target MathBenchmark --parallel 2 || fail "Build failed"
}
...
if $REBUILD || [ ! -f "$BUILD_DIR/CMakeCache.txt" ]; then
  ...
  NEED=false
  for src in \
    "$REPO_ROOT/AestraCore/include/AestraMath.h" \
    "$REPO_ROOT/AestraCore/src/MathTests.cpp" \
    "$REPO_ROOT/AestraCore/src/MathBenchmark.cpp" \
...
for candidate in \
  "$BUILD_DIR/MathTests" \
  "$BUILD_DIR/AestraCore/MathTests" \
  "$BUILD_DIR/bin/MathTests" \
  "$BUILD_DIR/bin/AestraCore/MathTests"; do
  [ -x "$candidate" ] && TESTS_BIN="$candidate" && break
done
for candidate in \
  "$BUILD_DIR/MathBenchmark" \
  "$BUILD_DIR/AestraCore/MathBenchmark" \
  "$BUILD_DIR/bin/MathBenchmark" \
  "$BUILD_DIR/bin/AestraCore/MathBenchmark"; do
  [ -x "$candidate" ] && BENCH_BIN="$candidate" && break
done

if [ -z "$TESTS_BIN" ] || [ -z "$BENCH_BIN" ]; then
  log "Missing binaries, rebuilding targets..."
  cmake --build "$BUILD_DIR" --target MathTests MathBenchmark --parallel 2 || fail "Build failed"
  for candidate in \
    "$BUILD_DIR/MathTests" \
    "$BUILD_DIR/AestraCore/MathTests" \
    "$BUILD_DIR/bin/MathTests" \
...
[ -z "$TESTS_BIN" ] && fail "Cannot find MathTests binary"
[ -z "$BENCH_BIN" ] && fail "Cannot find MathBenchmark binary"

log "Tests binary: $TESTS_BIN"
log "Benchmark binary: $BENCH_BIN"

# Lane 1: Correctness
log "=== Lane 1: Math Tests (Correctness) ==="
OUT_FILE="${RESULTS_DIR}/${RID}_math.txt"
set +e
...
# Lane 2: Benchmark
log "=== Lane 2: Math Benchmark (Performance) ==="
...
if [ "$TESTS_EXIT" -ne 0 ]; then
  DECISION="reject"
  HARD_FAILURES="[\"MathTests exited with code $TESTS_EXIT\"]"
fi

if [ "$BENCH_EXIT" -ne 0 ]; then
  DECISION="reject"
  if [ "$BENCH_EXIT" -eq 2 ]; then
    HARD_FAILURES="[\"MathBenchmark XRUN/miss rate exceeded thresholds\"]"
  else
    HARD_FAILURES="[\"MathBenchmark exited with code $BENCH_EXIT\"]"
  fi
fi
