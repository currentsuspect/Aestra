...
#include <cmath>
#include <algorithm>
#include "../../NomadCore/include/NomadJSON.h" // [NEW]
...
std::vector<uint8_t> SamplerPlugin::saveState() const {
     Nomad::JSON json = Nomad::JSON::object();
     
     // Params
     Nomad::JSON pArr = Nomad::JSON::array();
     for (const auto& p : m_params) {
         pArr.push(Nomad::JSON(static_cast<double>(p.load())));
     }
...
bool SamplerPlugin::loadState(const std::vector<uint8_t>& state) {
    ...
    // Simple JSON parsing
    Nomad::JSON json = Nomad::JSON::parse(s);
