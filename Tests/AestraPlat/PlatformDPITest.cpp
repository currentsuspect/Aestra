...
int main() {
    ...
    AESTRA_LOG_INFO("Physical size: " + std::to_string(static_cast<int>(800 * dpiScale)) + "x" + std::to_string(static_cast<int>(600 * dpiScale)));
    
    // Set up DPI change callback
    window->setDPIChangeCallback([](float newScale) {
        AESTRA_LOG_INFO("DPI changed! New scale: " + std::to_string(newScale));
    });
