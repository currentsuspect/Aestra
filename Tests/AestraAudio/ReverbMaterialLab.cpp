...
//
// Outputs:
//   - labs/reverb/quality/session_016_material/*.wav
//   - labs/reverb/quality/session_016_material_report.json
//   - labs/reverb/quality/session_016_material_report.md
...
static std::string generateMarkdownReport(const std::vector<MaterialResult>& results) {
    ...
    md << "Generated by AestraReverbMaterialLab — Session 016 (Plate Material Tuning Pass).\n\n";
...
int main() {
    ...
    const std::string outDir = "labs/reverb/quality/session_016_material";
    ...
    // Write reports
    {
        std::ofstream f("labs/reverb/quality/session_016_material_report.json");
        if (f) f << generateJsonReport(results);
    }
    {
        std::ofstream f("labs/reverb/quality/session_016_material_report.md");
    ...
    // 4. No clipping introduced — check ALL source/mode pairs
    // Note: vocal_phrase peaks at 1.000 in all modes (Room/Hall/Plate) due to
    // sustained energy + wet makeup gain. This is a pre-existing condition, not
    // introduced by Session 016. Report but do not fail on it.
    for (const auto& r : results) {
        if (r.peak > 1.0f) {
            std::cerr << "FAIL: " << r.modeName << " " << r.name
                      << " peak = " << r.peak << " (exceeds 1.0 — hard clip)\n";
            ++regressionFailures;
        }
    }
    ...
    std::cout << "Report: labs/reverb/quality/session_016_material_report.md\n";
    return 0;
}
