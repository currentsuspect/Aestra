...
// Performance benchmarking suite for AestraAudio.
...
void runPolyphonyBenchmark() {
    ...
    for (trackCount = 32; trackCount <= kMaxTracks; trackCount += kStep) {
        ...
        std::cout << "  " << std::setw(6) << trackCount << " | " << std::setw(13) << avgUs << " | " << std::setw(5)
                  << std::fixed << std::setprecision(1) << load << "% | " << (load > 90.0 ? "FAIL" : "OK") << std::endl;
    ...
    if (!failed)
        std::cout << "-> Amazing! Exceeded " << kMaxTracks << " tracks." << std::endl;
}
...
void runDSPBenchmark() {
    ...
    // Create filters
    std::vector<std::unique_ptr<Aestra::Audio::DSP::Filter>> filters;

    // Input/Output buffers
    std::vector<float> buffer(kBlockSize); // Mono processing for density
    for (auto& s : buffer)
        s = 0.5f; // DC offset to avoid denormals

    auto addFilters = [&](int count) {
        for (int i = 0; i < count; ++i) {
            auto f = std::make_unique<Aestra::Audio::DSP::Filter>(static_cast<float>(kSampleRate));
            f->setType(Aestra::Audio::DSP::FilterType::LowPass);
            f->setOversampling(Aestra::Audio::DSP::OversamplingFactor::TwoX);
            f->setCutoff(1000.0f);
    ...
    for (int count = 16; count <= 1024; count += 16) {
        ...
        std::cout << "  " << std::setw(7) << count << " | " << std::setw(13) << avgUs << " | " << std::setw(5)
                  << std::fixed << std::setprecision(1) << load << "% | " << (load > 90.0 ? "FAIL" : "OK") << std::endl;
...
int main(int argc, char** argv) {
    std::cout << "========================================" << std::endl;
    std::cout << " AESTRA AUDIO PRO PERFORMANCE SUITE v1.0" << std::endl;
    std::cout << "========================================" << std::endl;
