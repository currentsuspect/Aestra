...
void generateSineWave(std::vector<float>& buffer, uint32_t frames, uint32_t channels, uint32_t sampleRate,
...
void testUpsample() {
    ...
    const uint32_t expectedOutput =
        static_cast<uint32_t>(std::ceil(static_cast<double>(inputFrames) * 48000.0 / 44100.0));

    std::vector<float> input;
    generateSineWave(input, inputFrames, 2, 44100, 440.0);

    std::vector<float> output(expectedOutput * 2 + 1024); // Extra buffer
    uint32_t written = src.process(input.data(), inputFrames, output.data(), static_cast<uint32_t>(output.size() / 2));
    ...
    recordTest("Upsample produces correct frame count", written >= minExpected && written <= maxExpected,
               "Expected ~" + std::to_string(expectedOutput) + ", got " + std::to_string(written));

    recordTest("Latency is reported", src.getLatency() > 0, "Latency: " + std::to_string(src.getLatency()) + " frames");
}
...
void testDownsample() {
    ...
    const uint32_t expectedOutput =
        static_cast<uint32_t>(std::ceil(static_cast<double>(inputFrames) * 44100.0 / 48000.0));
    ...
    recordTest("Downsample produces correct frame count", written >= minExpected && written <= maxExpected,
               "Expected ~" + std::to_string(expectedOutput) + ", got " + std::to_string(written));
}
...
void testRoundTrip() {
    ...
    const uint32_t upFrames =
        Aestra::Audio::estimateOutputFrames(frames, originalRate, intermediateRate, up.getLatency());
    std::vector<float> upsampled(upFrames * 2);
    uint32_t upWritten = up.process(original.data(), frames, upsampled.data(), upFrames);

    // Downsample 48000 -> 44100
    SampleRateConverter down;
    down.configure(intermediateRate, originalRate, 2, SRCQuality::Sinc16);

    const uint32_t downFrames =
        Aestra::Audio::estimateOutputFrames(upWritten, intermediateRate, originalRate, down.getLatency());
    std::vector<float> roundTrip(downFrames * 2);
    uint32_t downWritten = down.process(upsampled.data(), upWritten, roundTrip.data(), downFrames);
    ...
    if (compareFrames > 100) {
        const double rmsError = calculateRMSError(original.data() + compareStart * 2,
                                                  roundTrip.data() + compareStart * 2, compareFrames * 2);

        const float maxError = calculateMaxError(original.data() + compareStart * 2,
                                                 roundTrip.data() + compareStart * 2, compareFrames * 2);

        std::cout << "  Round-trip RMS error: " << std::scientific << std::setprecision(4) << rmsError << std::endl;
        std::cout << "  Round-trip max error: " << std::fixed << std::setprecision(6) << maxError << std::endl;

        // For Sinc16 quality with phase shift from double conversion,
        // expect reasonable (not perfect) round-trip fidelity
        recordTest("Round-trip RMS error < 0.15", rmsError < 0.15, "RMS: " + std::to_string(rmsError));
        recordTest("Round-trip max error < 0.20", maxError < 0.20f, "Max: " + std::to_string(maxError));
    } else {
        recordTest("Round-trip comparison", false, "Not enough frames for comparison");
    }
}
...
void testQualityLevels() {
    ...
    std::array<SRCQuality, 5> levels = {SRCQuality::Linear, SRCQuality::Cubic, SRCQuality::Sinc8, SRCQuality::Sinc16,
                                        SRCQuality::Sinc64};
...
void testPerformance() {
    ...
    std::cout << "  Processed " << inputFrames << " frames in " << std::fixed << std::setprecision(3)
              << seconds * 1000.0 << " ms\n";
    std::cout << "  Real-time factor: " << std::fixed << std::setprecision(1) << realtimeFactor << "x\n";

    recordTest("Performance >= 10x real-time", realtimeFactor >= 10.0, std::to_string(realtimeFactor) + "x real-time");
}
...
void testSIMDMatchesScalar() {
    ...
    const uint32_t outCap = Aestra::Audio::estimateOutputFrames(inputFrames, srcRate, dstRate, 64);
...
void testMultiChannel() {
    ...
    recordTest("6-channel processing works", written > 0, "Wrote " + std::to_string(written) + " frames");
...
void testReset() {
    ...
    recordTest("Reset produces consistent output", written1 == written2,
               "Before: " + std::to_string(written1) + ", After: " + std::to_string(written2));
}
...
void testVariableRatio() {
    ...
    recordTest("Ratio approaches target after processing", std::abs(src.getCurrentRatio() - 1.0) < 1.5,
...
int main() {
    std::cout << "=========================================\n";
    std::cout << "  Aestra SampleRateConverter Test Suite\n";
    std::cout << "=========================================\n";
