...
double peak(const std::vector<float>& data) {
    double value = 0.0;
    for (float sample : data) value = std::max(value, std::abs(static_cast<double>(sample)));
    return value;
}
...
bool meetsThresholds(const CaseResult& result, const CaseThresholds& thresholds) {
    return result.nanInfCount == 0 &&
           result.peakOut <= thresholds.maxPeakOut &&
           result.rumbleDeltaDb <= thresholds.maxRumbleDeltaDb &&
           result.mudDeltaDb <= thresholds.maxMudDeltaDb &&
           result.presenceDeltaDb >= thresholds.minPresenceDeltaDb &&
           result.airDeltaDb >= thresholds.minAirDeltaDb;
}

CaseResult analyzeMaterial(const std::string& name,
                           const std::vector<float>& input,
                           void (*configure)(AestraEQ&),
...
bool writeMarkdown(const std::string& path, const std::vector<CaseResult>& results) {
    ...
    for (const auto& result : results) {
        out << "| " << result.name << " | "
            << fixed(result.peakIn) << " | " << fixed(result.rmsIn) << " | "
            << fixed(result.peakOut) << " | " << fixed(result.rmsOut) << " | "
            << fixed(result.rumbleDeltaDb, 2) << " dB | "
            << fixed(result.mudDeltaDb, 2) << " dB | "
            << fixed(result.presenceDeltaDb, 2) << " dB | "
            << fixed(result.airDeltaDb, 2) << " dB | "
...
int main() {
    ...
    results.push_back(analyzeMaterial("cleanup_mix", generatedMix(), configureCleanupEQ,
                                      thresholds(-7.0, -3.0, 1.5, 0.8)));
    ...
    results.push_back(analyzeMaterial("tilt_brighten", tiltMaterial(), configureTiltBrightenEQ,
                                      thresholds(0.5, -2.0, 3.0, 4.0)));
    ...
    for (const auto& result : results) {
        ...
        std::cout << result.name
                  << ": rumble=" << fixed(result.rumbleDeltaDb, 2)
                  << "dB mud=" << fixed(result.mudDeltaDb, 2)
                  << "dB presence=" << fixed(result.presenceDeltaDb, 2)
                  << "dB air=" << fixed(result.airDeltaDb, 2)
                  << "dB sane=" << (result.sane ? "yes" : "no") << "\n";
    }
    return allSane ? 0 : 1;
}
