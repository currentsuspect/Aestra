...
public:
    ...
    PlaylistLaneID getClipLane() {
        return m_model.findClipLane(m_clipId);
    }
...
bool testExecuteMovesClip() {
    ...
    auto cmd = std::make_shared<MoveClipCommand>(
        fixture.getModel(), fixture.getClipId(), 20.0, PlaylistLaneID());
...
bool testMoveToDifferentLane() {
    ...
    // Move to lane 2
    auto cmd = std::make_shared<MoveClipCommand>(
        fixture.getModel(), fixture.getClipId(), 10.0, lane2);
...
bool testMoveWithBeatAndLaneChange() {
    ...
    auto cmd = std::make_shared<MoveClipCommand>(
        fixture.getModel(), fixture.getClipId(), 25.0, lane2);
...
bool testInvalidClipId() {
    ...
    auto cmd = std::make_shared<MoveClipCommand>(
        fixture.getModel(), fakeId, 20.0, PlaylistLaneID());
...
bool testHistoryIntegration() {
    ...
    // Move via history
    auto cmd = std::make_shared<MoveClipCommand>(
        fixture.getModel(), fixture.getClipId(), 30.0, PlaylistLaneID());
...
bool testMultipleMoves() {
    ...
    for (double beat = 20.0; beat <= 40.0; beat += 10.0) {
        auto cmd = std::make_shared<MoveClipCommand>(
            fixture.getModel(), fixture.getClipId(), beat, PlaylistLaneID());
        history.pushAndExecute(cmd);
    }
