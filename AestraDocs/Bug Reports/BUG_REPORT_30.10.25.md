...
2. Launch **Aestra** with the device set to **WASAPI Exclusive**.
3. Hit Play in Aestra (without switching to Shared).

**Expected**

* If the device is already locked by another Exclusive client, Aestra:
...
* With FL holding Exclusive, starting playback in Aestra **never** emits harsh/loud audio.
* Aestra either:
...
* QA checklist:
  ...
  * FL in Exclusive → Aestra Exclusive: toast + silence **or** clean auto-Shared.
  * Release device in FL; retry Exclusive in Aestra succeeds cleanly (no pop).
...
* Piano Roll compiles cleanly across targets; links into Aestra UI without undefined references; headers are self-contained.
...
* CMake target not declared as a proper library or not linked to its deps (e.g., AestraCore, AestraUI, math, containers).
...
* CMake hygiene (example):
  ...
  ```cmake
  add_library(AestraPianoRoll STATIC
    src/PianoRoll.cpp
    src/NoteGrid.cpp
    src/Selection.cpp
  ...
  target_link_libraries(AestraPianoRoll
    PUBLIC AestraCore AestraUI
    PRIVATE AestraMath AestraContainers
  )
  target_compile_features(AestraPianoRoll PUBLIC cxx_std_20)
  ```
* Header discipline:
...
* Linkage:
  ...
  * Ensure AestraUI links **against** `AestraPianoRoll`; do not duplicate sources across targets.
  * Consolidate utility singletons to one TU to avoid ODR.
* Config:

  * Align compile options/defines across targets (exceptions/RTTI, `_UNICODE`, `Aestra_PLATFORM_*`).
...
* QA:
  ...
  * Run tests in CI; artifacts include `AestraPianoRoll.lib/.a` and executable with Piano Roll integrated.
