# AestraUI Coordinate System Guide

## Overview

This document explains the coordinate system used in AestraUI and critical patterns for positioning components correctly. Understanding this is essential for avoiding layout issues, especially when working with nested components.
...
**AestraUI uses a standard 2D screen coordinate system:**
...
**AestraUI does NOT automatically transform child component coordinates relative to their parent.**
...
2. **AestraContent** set transport bar bounds to `(0, 0)` (relative positioning)
...
2. **Don't assume relative coordinates** - they don't exist in AestraUI
...
To make absolute positioning more ergonomic, AestraUI provides utility helpers in `NUITypes.h`:
...
Aestra UI now supports comprehensive YAML-based configuration for pixel-perfect control over all visual aspects:
...
**Remember:** AestraUI uses absolute screen coordinates for all components. Always add parent offsets when positioning children, and never reset position in `onResize()`. Use the utility helpers (`NUIAbsolute`, `NUICentered`, etc.) to make positioning cleaner and less error-prone. Render order determines stacking (Z-order).
