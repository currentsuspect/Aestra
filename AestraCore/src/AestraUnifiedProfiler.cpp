...
/**
 * @file AestraUnifiedProfiler.cpp
...
ScopedTimer::ScopedTimer(const char* name) : m_name(name), m_start(std::chrono::steady_clock::now()) {
...
UnifiedProfiler::UnifiedProfiler()
    : m_frameStart(std::chrono::steady_clock::now()), m_renderEnd(m_frameStart), m_swapEnd(m_frameStart),
      m_lastFrameEnd(m_frameStart), m_fpsTimer(m_frameStart) {
    ...
    m_exportMetadata.buildInfo = "AESTRA-2025-Core";
...
void UnifiedProfiler::endFrame() {
    ...
    m_currentFrame.cpuTimeMs =
        (m_currentFrame.uiUpdateUs + m_currentFrame.renderPrepUs + m_currentFrame.inputPollUs) / 1000.0;
...
void UnifiedProfiler::updatePerformanceAlerts() {
    ...
    if (m_currentFrame.totalTimeMs > m_thresholds.frameTimeMs) {
        ...
        alert.message = "Frame time spike: " + std::to_string(m_currentFrame.totalTimeMs) +
                        "ms (threshold: " + std::to_string(m_thresholds.frameTimeMs) + "ms)";
    ...
    if (m_currentFrame.audioLoadPercent > m_thresholds.audioLoadPercent) {
        ...
        alert.message = "High audio load: " + std::to_string(m_currentFrame.audioLoadPercent) +
                        "% (threshold: " + std::to_string(m_thresholds.audioLoadPercent) + "%)";
...
void UnifiedProfiler::exportToJSON(const std::string& filepath) {
    ...
    file << "    {\"name\": \"Metadata\", \"cat\": \"metadata\", \"ph\": \"i\", "
         ...
         << "\"exportTime\": \""
         << std::chrono::duration_cast<std::chrono::seconds>(m_exportMetadata.exportTime.time_since_epoch()).count()
         << "\"}}\n";
    ...
    for (size_t i = 0; i < m_history.size(); ++i) {
        ...
        for (const auto& z : frame.zones) {
            file << ",\n";
            uint64_t offset = 0;
            if (z.startUs >= frame.frameStartUs)
                offset = z.startUs - frame.frameStartUs;
...
void UnifiedProfiler::exportToHTML(const std::string& filepath) {
    ...
    file << "<!DOCTYPE html>\n";
    file << "<html>\n<head>\n";
    file << "<title>Aestra Profiler Report</title>\n";
    file << "<style>\n";
    ...
    file << "<h1>Aestra Performance Report</h1>\n";
    file << "<p>Generated: "
         << std::chrono::duration_cast<std::chrono::seconds>(m_exportMetadata.exportTime.time_since_epoch()).count()
         << "</p>\n";
    ...
    file << "<div class=\"metric "
         ...
         << "\">\n";
    file << "<strong>Frame Time:</strong> " << std::fixed << std::setprecision(2) << m_currentFrame.totalTimeMs
         << " ms (FPS: " << m_fps << ")</div>\n";

    file << "<div class=\"metric "
         << (m_currentFrame.audioLoadPercent < 70 ? "good"
                                                  : (m_currentFrame.audioLoadPercent < 90 ? "warning" : "error"))
         << "\">\n";
    file << "<strong>Audio Load:</strong> " << std::fixed << std::setprecision(1) << m_currentFrame.audioLoadPercent
         << "%</div>\n";
...
void UnifiedProfiler::printFrameStats() const {
    ...
    std::cout
        << "\n┌────────────────────────────────────────────────────────────────────────────────────────────────────┐\n";
    std::cout << "│                                    AESTRA UNIFIED PROFILER - Frame #" << std::setw(9)
              << m_frameCount << "                                   │\n";
    std::cout
        << "├────────────────────────────────────────────────────────────────────────────────────────────────────┤\n";
    ...
    if (m_currentFrame.renderTimeMs > 50.0)
        std::cout << "⚠️  CPU bottleneck!        │\n";
    else
        std::cout << "                              │\n";

    std::cout << "│   Swap Time:    " << std::setw(8) << m_currentFrame.swapTimeMs << " ms  ";
    if (m_currentFrame.swapTimeMs > 20.0)
        std::cout << "⚠️  VSync stall!        │\n";
    else
        std::cout << "                      │\n";
    ...
    std::cout << "│   Total Time:   " << std::setw(8) << m_currentFrame.totalTimeMs
              << " ms                               │\n";
    std::cout << "│   FPS:          " << std::setw(8) << std::setprecision(1) << m_fps
              << "                                    │\n";

    // Audio and memory metrics
    std::cout << "│   Audio Load:   " << std::setw(8) << std::setprecision(1) << m_currentFrame.audioLoadPercent
              << "%  ";
    if (m_currentFrame.audioLoadPercent > 90.0)
        std::cout << "🔴 Audio overload!      │\n";
    else if (m_currentFrame.audioLoadPercent > 70.0)
        std::cout << "🟡 High audio load    │\n";
    else
        std::cout << "🟢 Healthy audio    │\n";
    ...
    std::cout
        << "│ AVERAGES (smoothed):                                                                                │\n";
    std::cout << "│   Render:       " << std::setw(8) << std::setprecision(2) << m_averageStats.renderTimeMs
              << " ms                               │\n";
    ...
    std::cout << "│   FPS:          " << std::setw(8) << std::setprecision(1) << m_fps
              << "                                    │\n";
    std::cout << "│   Audio:        " << std::setw(8) << std::setprecision(1) << m_averageStats.audioLoadPercent
              << "%                               │\n";
...
void UnifiedProfiler::printPerformanceSummary() const {
    std::cout << "\n═══════════════════════════════════════════════════════════════════════════════\n";
    std::cout << "                              AESTRA PERFORMANCE SUMMARY                              \n";
    std::cout << "═══════════════════════════════════════════════════════════════════════════════\n";
    ...
    if (!m_regressions.empty()) {
        ...
        for (const auto& reg : m_regressions) {
            std::cout << "  - " << reg.metricName << ": " << std::setprecision(1) << reg.regressionPercent
                      << "% regression\n";
        }
    }
