...
bool testFile() {
    ...
    const std::string testContent = "Hello, AESTRA!";
...
bool testBinarySerialization() {
    ...
    writer.write(std::string("AESTRA"));
...
bool testJSON() {
    ...
    TEST_ASSERT(obj["name"].asString() == "AESTRA", "Name should be 'AESTRA'");
    ...
    // Test parsing
    std::string testJson = R"({"name":"AESTRA","version":1.0,"active":true,"tags":["audio","daw"]})";
    JSON parsed = JSON::parse(testJson);
    TEST_ASSERT(parsed.isObject(), "Should parse object");
    TEST_ASSERT(parsed["name"].asString() == "AESTRA", "Should parse name");
...
int main() {
    std::cout << "\n==================================" << std::endl;
    std::cout << "  AestraCore File I/O Tests" << std::endl;
    std::cout << "==================================" << std::endl;
