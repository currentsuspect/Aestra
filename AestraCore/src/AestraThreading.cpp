...
RealTimeThreadPool::RealTimeThreadPool(size_t numThreads) : m_stop(false), m_taskCount(0), m_activeTasks(0) {
    ...
    for (size_t i = 0; i < numThreads; ++i) {
        m_workers.emplace_back(&RealTimeThreadPool::workerLoop, this, static_cast<uint32_t>(i));
    }
}
...
size_t RealTimeThreadPool::size() const { return m_workers.size(); }
