...
#ifdef _WIN32
    // 1. Lazy load the function pointer (thread-safe static init)
    static auto pAvSetMmThreadCharacteristicsA = []() {
        HMODULE hAvrt = LoadLibraryA("Avrt.dll");
        if (hAvrt) {
            return (HANDLE (WINAPI *)(LPCSTR, LPDWORD))GetProcAddress(hAvrt, "AvSetMmThreadCharacteristicsA");
        }
        return (HANDLE (WINAPI *)(LPCSTR, LPDWORD))nullptr;
    }();

    // 2. Call it for the current thread
    if (pAvSetMmThreadCharacteristicsA) {
        DWORD taskIndex = 0;
        HANDLE hTask = pAvSetMmThreadCharacteristicsA("Pro Audio", &taskIndex);
...
ThreadPool::ThreadPool(size_t numThreads) : stop(false) {
    // If numThreads is 0 (hardware_concurrency returned 0), default to 1
...
RealTimeThreadPool::RealTimeThreadPool(size_t numThreads)
    ...
    // If numThreads is 0, default to 1
    if (numThreads == 0) numThreads = 1;
...
void RealTimeThreadPool::dispatch(uint32_t count, void* context, void** taskDataArray, TaskFunc func, Barrier* syncBarrier) {
    m_taskFunc = func;
    m_context = context;
...
size_t RealTimeThreadPool::size() const {
...
void RealTimeThreadPool::workerLoop(uint32_t threadIdx) {
    (void)threadIdx;
    while (!m_stop) {
        m_signal.wait([this] { return m_stop || m_taskCounter.load(std::memory_order_acquire) < (int)m_taskCount; });
