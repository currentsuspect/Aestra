...
Profiler::Profiler()
    : m_frameStart(std::chrono::steady_clock::now()), m_cpuStart(m_frameStart), m_gpuStart(m_frameStart),
...
void Profiler::exportToJSON(const std::string& filepath) {
    ...
    for (size_t i = 0; i < m_history.size(); ++i) {
        ...
        for (const auto& z : frame.zones) {
            ...
            uint64_t dur = 0;
            if (z.endUs >= z.startUs)
                dur = z.endUs - z.startUs;
