...
bool testConfig() {
    ...
    // Test version
    std::cout << "  Version: " << AESTRA_VERSION_STRING << std::endl;
    TEST_ASSERT(AESTRA_VERSION_MAJOR == 0, "Major version should be 0");
    TEST_ASSERT(AESTRA_VERSION_MINOR == 1, "Minor version should be 1");
    TEST_ASSERT(AESTRA_VERSION_PATCH == 0, "Patch version should be 0");

    // Test utility macros
    const char* test = AESTRA_STRINGIFY(AESTRA);
    TEST_ASSERT(std::string(test) == "AESTRA", "Stringify macro should work");

    int testArray[] = {1, 2, 3, 4, 5};
    TEST_ASSERT(AESTRA_ARRAY_SIZE(testArray) == 5, "Array size macro should work");
...
bool testAsserts() {
    ...
    AESTRA_STATIC_ASSERT(true, "This should always pass");

    // Test verify (always enabled)
    AESTRA_VERIFY(true);
    AESTRA_VERIFY_MSG(1 == 1, "One equals one");

    // Test compiler hints
    int x = 10;
    if (AESTRA_LIKELY(x > 0)) {
        // Likely branch
    }
    if (AESTRA_UNLIKELY(x < 0)) {
        // Unlikely branch
    }
...
// Force Inline Test
// =============================================================================
AESTRA_FORCE_INLINE int forceInlinedFunction(int a, int b) {
    return a + b;
}

AESTRA_NO_INLINE int noInlineFunction(int a, int b) {
    return a * b;
}
...
int main() {
    std::cout << "\n==================================" << std::endl;
    std::cout << "  AestraCore Config & Assert Tests" << std::endl;
    std::cout << "==================================" << std::endl;
