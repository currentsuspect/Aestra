 ...
 * 
 * Features:
 * - Zone timing macros (AESTRA_ZONE) - replaces ScopedTimer
...
public:
    ...
    // Zone timing (replaces old AestraProfiler)
...
// Memory profiling macros
    ...
    #define AESTRA_MEMORY_ALLOC(size) Aestra::UnifiedProfiler::getInstance().recordMemoryAllocation(size)
    #define AESTRA_MEMORY_FREE(size) Aestra::UnifiedProfiler::getInstance().recordMemoryDeallocation(size)
#else
    #define AESTRA_MEMORY_ALLOC(size) ((void)0)
    #define AESTRA_MEMORY_FREE(size) ((void)0)
#endif