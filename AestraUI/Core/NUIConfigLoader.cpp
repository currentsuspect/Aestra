...
void NUIConfigLoader::saveConfig(const std::string& filePath) {
    ...
    Aestra::JSON config = Aestra::JSON::object();

    // Save colors
    Aestra::JSON colors = Aestra::JSON::object();
    ...
    // Save layout
    Aestra::JSON layout = Aestra::JSON::object();
    ...
    // Save spacing
    Aestra::JSON spacing = Aestra::JSON::object();
...
void NUIConfigLoader::setNestedValue(Aestra::JSON& json, const std::string& path, const std::string& value) {
    ...
    Aestra::JSON* current = &json;
    for (size_t i = 0; i < keys.size() - 1; i++) {
        if (current->isObject()) {
            if (!current->has(keys[i])) {
                current->set(keys[i], Aestra::JSON::object());
            }
...
void NUIConfigLoader::applyColors(const Aestra::JSON& colors) {
    auto& themeManager = NUIThemeManager::getInstance();
    auto& theme = themeManager.getCurrentThemeMutable();
...
void NUIConfigLoader::applyLayout(const Aestra::JSON& layout) {
    auto& themeManager = NUIThemeManager::getInstance();
    auto& theme = themeManager.getCurrentThemeMutable();
...
void NUIConfigLoader::applyTypography(const Aestra::JSON& typography) {
    auto& themeManager = NUIThemeManager::getInstance();
    auto& theme = themeManager.getCurrentThemeMutable();
...
float NUIConfigLoader::parseDimension(const Aestra::JSON& value) {
    if (value.isNumber()) {
