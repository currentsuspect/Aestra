...
void AestraVerbEditor::buildControls() {
    ...
    struct Meta { const char* label; } meta[] = {{"Decay"},{"Damping"},{"Predelay"},{"Width"},{"Mix"}};
    uint32_t ids[] = {0, 1, 2, 3, 4};
    for (int i = 0; i < 5; ++i) {
...
void AestraVerbEditor::drawKnob(NUIRenderer& renderer, const Knob& k, NUIColor accent) {
    ...
    float startAngle = pi * 0.75f, endAngle = startAngle + k.value * pi * 1.5f;
    for (int i = 0; i < 32; ++i) {
        float a1 = startAngle + (endAngle - startAngle) * i / 32;
        float a2 = startAngle + (endAngle - startAngle) * (i + 1) / 32;
        renderer.drawLine({cx + std::cos(a1)*(r-3), cy + std::sin(a1)*(r-3)},
                          {cx + std::cos(a2)*(r-3), cy + std::sin(a2)*(r-3)}, 3.0f, accent.withAlpha(0.8f));
    }
    float pa = startAngle + k.value * pi * 1.5f;
