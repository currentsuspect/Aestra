...
- **Innovation**: Apply graph coloring algorithms to the audio dependency graph to identify non-overlapping task sets.
- **Benefit**: Maximizes cache locality and minimizes thread synchronization overhead during parallel rendering.
...
- **Plan**: Integrate `SimdLin`, a lightweight, header-only SIMD linear algebra library.
- **Benefit**: Accelerates NeuralFX inference and matrix-heavy DSP (EQs, Compressors) by 4-8x using AVX2/AVX-512 intrinsics.
...
- **Plan**: Implement "T-Console" summation algorithm which introduces micro-variations in pitch, phase, and gain across channels.
- **Benefit**: Emulates the non-deterministic "gluing" effect of analog mixing consoles, reducing the sterility of digital summing.

### Spectral Anti-Aliasing

- **Plan**: Detect and suppress aliasing harmonics generated by non-linear saturation using spectral tracking.
- **Benefit**: Cleaner distortion and saturation textures without the heavy CPU cost of high-factor oversampling.
