...
void PatternPlaybackEngine::schedulePatternInstance(PatternID pid, double startBeat, uint32_t instanceId) {
    ...
    Aestra::Log::info("[PatternPlayback] Scheduled instance " + std::to_string(instanceId) + 
...
void PatternPlaybackEngine::refillWindow(uint64_t currentFrame, int sampleRate, int lookaheadSamples) {
    ...
    for (auto& inst : m_activeInstances) {
        ...
        if (!pattern) {
             if (shouldLog) Aestra::Log::info("[PatternPlayback] Pattern not found: " + std::to_string(inst.patternId.value));
             continue;
        }
        if (!pattern->isMidi()) {
             if (shouldLog) Aestra::Log::info("[PatternPlayback] Pattern is not MIDI: " + std::to_string(inst.patternId.value));
             continue;
        }
