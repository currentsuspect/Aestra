...
bool CLAPPluginInstance::load(const std::filesystem::path& path, int pluginIndex) {
    ...
    // Get entry point
    auto entryFunc = reinterpret_cast<const clap_plugin_entry* (*)()>(
        GetProcAddress(static_cast<HMODULE>(m_library), "clap_entry"));
    if (!entryFunc) {
        // Try direct symbol
    ...
    if (desc->features) {
        for (const char* const* f = desc->features; *f; ++f) {
            if (strcmp(*f, CLAP_PLUGIN_FEATURE_INSTRUMENT) == 0 ||
                strcmp(*f, CLAP_PLUGIN_FEATURE_SYNTHESIZER) == 0) {
    ...
    // Check for GUI
    auto gui = static_cast<const clap_plugin_gui*>(
...
void CLAPPluginInstance::activate() {
    if (!m_loaded || m_active || !m_plugin) return;
...
void CLAPPluginInstance::process(
...
bool CLAPPluginInstance::openEditor(void* parentWindow) {
    if (!m_plugin || m_editorOpen) return false;
...
void CLAPPluginInstance::closeEditor() {
    if (!m_plugin || !m_editorOpen) return;
...
uint32_t CLAPPluginInstance::getLatencySamples() const {
    ...
    auto latency = static_cast<const clap_plugin_latency*>(
...
uint32_t CLAPPluginInstance::getTailSamples() const {
    ...
    auto tail = static_cast<const clap_plugin_tail*>(
            ...
            if (desc) {
                ...
                if (desc->features) {
                    for (const char* const* f = desc->features; *f; ++f) {
                        ...
                        if (!info.category.empty()) info.category += "|";
                        info.category += *f;
                    }
                }
