...
ASIO::ASIOCallbacks ASIODriver::s_asioCallbacks = {wrapper_bufferSwitch, wrapper_sampleRateDidChange,
                                                   wrapper_asioMessage, wrapper_bufferSwitchTimeInfo};
            ...
            if (RegOpenKeyExA(hKey, keyName, 0, KEY_READ, &hSubKey) == ERROR_SUCCESS) {
                ...
                if (RegQueryValueExA(hSubKey, "CLSID", nullptr, &type, (LPBYTE)clsidStr, &clsidLen) == ERROR_SUCCESS) {
                    ...
                    devices.push_back({devId,
                                       std::string(descStr),
                                       2,
                                       2,                            // Default capability guess (refined on open)
...
bool ASIODriver::loadDriver(uint32_t deviceIndex) {
    ...
    if (m_asio->init(hwnd) != 0)
        return true;
    if (m_asio->init(nullptr) != 0)
        return true;
...
bool ASIODriver::openStream(const AudioStreamConfig& config, AudioCallback callback, void* userData) {
    ...
    if (m_bufferSize < minSize)
        m_bufferSize = minSize; // Clamp to valid range
    ...
    ASIO::ASIOError err = (ASIO::ASIOError)m_asio->createBuffers(m_bufferInfos.data(), (long)m_bufferInfos.size(),
                                                                 m_bufferSize, &s_asioCallbacks);
    ...
    for (int i = 0; i < m_inputChannels; ++i) {
        ...
        case ASIO::ASIOSTInt32LSB: {
            ...
            for (long f = 0; f < m_bufferSize; ++f) {
                m_interleavedInput[f * m_configuredInputChannels + i] =
                    static_cast<float>(src32[f]) * (1.0f / 2147483648.0f);
            }
            break;
        }
        case ASIO::ASIOSTInt16LSB: {
            ...
            for (long f = 0; f < m_bufferSize; ++f) {
                m_interleavedInput[f * m_configuredInputChannels + i] =
                    static_cast<float>(src16[f]) * (1.0f / 32768.0f);
            }
            break;
        }
