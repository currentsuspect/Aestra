...
namespace Audio {
        ...
        for (const auto& track : state.renderTracks) {
            ...
            if (!ctx.isOffline && track.selfBuffer && snaps && slotMap && track.trackIndex < ctx.graph->tracks.size()) {
                ...
                if (slotIdx != ChannelSlotMap::INVALID_SLOT && graphTrack.trackId != 0) {
                    ...
                    if (den > 1e-9) correlation = static_cast<float>(sumLR / den);
    ...
    void AudioRenderer::renderClipAudio(double* outputBuffer, TrackRTState& state, uint32_t trackIndex, const Context& ctx, AudioEngine& engineRef) {
        ...
        for (const auto& clip : clips) {
             if (!clip.audioData || blockEnd <= clip.startSample || blockStart >= clip.endSample) continue;
             ...
             if (clip.totalFrames > 0) {
                 if (phase >= static_cast<double>(clip.totalFrames)) continue;
                 framesToRender = std::min(framesToRender, static_cast<uint32_t>((static_cast<double>(clip.totalFrames) - phase) / ratio));
             }
             ...
             if (std::abs(ratio - 1.0) < 1e-9) {
                 ...
                 for (uint32_t i = 0; i < framesToRender; ++i) {
                     double fade = 1.0;
                     if (start + i < clip.startSample + CLIP_EDGE_FADE_SAMPLES) fade = std::min(fade, (double)(start+i-clip.startSample)/CLIP_EDGE_FADE_SAMPLES);
                     if (start + i + CLIP_EDGE_FADE_SAMPLES > clip.endSample) fade = std::min(fade, (double)(clip.endSample-(start+i))/CLIP_EDGE_FADE_SAMPLES);
                 ...
                 if (q == Interpolators::InterpolationQuality::Cubic) processResample(Interpolators::CubicInterpolator::interpolate);
                 else if (q == Interpolators::InterpolationQuality::Sinc64) processResample(Interpolators::Sinc64Turbo::interpolate);
                 else processResample(Interpolators::Sinc32Turbo::interpolate);
             }
        }
    }

    void AudioRenderer::processTrackEffects(const RenderTrack& track, AudioGraphState& graphState, uint32_t numFrames, uint32_t bufferOffset, AudioEngine& engineRef) {
        ...
        if (track.trackIndex < graph.tracks.size()) {
            ...
            state.gainL.setTarget(gainL); state.gainR.setTarget(gainR);
        }
        ...
        state.gainL.snap(); state.gainR.snap();
    }
    ...
    void AudioRenderer::processArsenalMidi(const Context& ctx, AudioEngine& engineRef) {
        ...
        if (!pe || !um || ctx.sampleRate == 0) return;
        auto snap = um->getAudioSnapshot();
        if (!snap || snap->units.empty()) return;
    ...
    void AudioRenderer::renderArsenalUnitsForTrack(uint32_t trackIndex, double* trackBuffer, const Context& ctx, AudioEngine& engineRef) {
        ...
        for (const auto& u : snap->units) {
            if (u.enabled && u.plugin && u.routeId == (int)trackIndex) {
                ...
                MidiBuffer* mIn = (bIdx < engineRef.m_unitMidiBuffers.size()) ? &engineRef.m_unitMidiBuffers[bIdx] : nullptr;
                MidiBuffer mOut;
