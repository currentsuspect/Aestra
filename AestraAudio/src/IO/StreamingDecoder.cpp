...
#define AESTRA_HAS_PREFETCH 1
#define AESTRA_PREFETCH(addr) _mm_prefetch(reinterpret_cast<const char*>(addr), _MM_HINT_T0)
#elif defined(__ARM_NEON) || defined(__ARM_NEON__) || defined(__aarch64__)
#define AESTRA_HAS_PREFETCH 1
#define AESTRA_PREFETCH(addr) __builtin_prefetch(addr, 0, 3)
#else
#define AESTRA_PREFETCH(addr) ((void)0)
#endif
...
AudioRingBuffer::AudioRingBuffer(size_t capacityFrames, uint32_t numChannels)
    : m_capacityFrames(capacityFrames), m_numChannels(numChannels) {
...
size_t AudioRingBuffer::write(const float* samples, size_t numFrames) {
    ...
    std::memcpy(&m_buffer[writeIdx * samplesPerFrame], samples, firstPart * samplesPerFrame * sizeof(float));

    if (toWrite > firstPart) {
        size_t secondPart = toWrite - firstPart;
        std::memcpy(&m_buffer[0], samples + firstPart * samplesPerFrame, secondPart * samplesPerFrame * sizeof(float));
    }
...
size_t AudioRingBuffer::read(float* samples, size_t numFrames) {
    ...
    for (size_t offset = 0; offset < firstPart * samplesPerFrame * sizeof(float); offset += prefetchStride) {
        AESTRA_PREFETCH(reinterpret_cast<const char*>(srcPtr) + offset);
    }

    std::memcpy(samples, srcPtr, firstPart * samplesPerFrame * sizeof(float));

    if (toRead > firstPart) {
        size_t secondPart = toRead - firstPart;
        std::memcpy(samples + firstPart * samplesPerFrame, &m_buffer[0], secondPart * samplesPerFrame * sizeof(float));
    }
...
size_t AudioRingBuffer::peek(float* samples, size_t numFrames) const {
    ...
    std::memcpy(samples, &m_buffer[readIdx * samplesPerFrame], firstPart * samplesPerFrame * sizeof(float));
...
size_t StreamingDecoder::read(float* output, size_t numFrames) {
    ...
    if (framesRead < numFrames) {
        ...
        std::memset(output + framesRead * channels, 0, (numFrames - framesRead) * channels * sizeof(float));
    }
...
void StreamingDecoder::decodeThreadFunc(const std::string& path, double targetLatencyMs) {
    ...
    double duration = (sampleRate > 0 && lengthFrames > 0) ? static_cast<double>(lengthFrames) / sampleRate : 0.0;
    ...
    if (preBufferFrames < 1024)
        preBufferFrames = 1024; // Minimum start threshold
    ...
    while (!m_stopRequested.load(std::memory_order_relaxed)) {
        ...
        if (chunkBuffer.size() < chunkSize * channels)
            chunkBuffer.resize(chunkSize * channels);
    ...
    m_state.store(State::Error, std::memory_order_release);
    if (m_onError)
        m_onError("Miniaudio disabled, cannot stream.");
#endif
}
