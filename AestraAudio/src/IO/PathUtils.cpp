...
std::string wideToUtf8(const std::wstring& wide) {
    ...
    WideCharToMultiByte(CP_UTF8, 0, wide.c_str(), static_cast<int>(wide.size()), utf8.data(), utf8Len, nullptr,
                        nullptr);
    return utf8;
#else
...
std::wstring utf8ToWide(const std::string& utf8) {
    ...
    MultiByteToWideChar(CP_UTF8, 0, utf8.c_str(), static_cast<int>(utf8.size()), wide.data(), wideLen);
    return wide;
#else
    std::wstring result;
...
std::wstring pathStringToWide(const std::string& path) {
    ...
    int wideLen = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, // Fail on invalid UTF-8
                                      path.c_str(), static_cast<int>(path.size()), nullptr, 0);

    if (wideLen > 0) {
        std::wstring wide(static_cast<size_t>(wideLen), L'\0');
        MultiByteToWideChar(CP_UTF8, 0, path.c_str(), static_cast<int>(path.size()), wide.data(), wideLen);
        return wide;
    }
    ...
    wideLen = MultiByteToWideChar(CP_ACP, 0, // ANSI code page
                                  path.c_str(), static_cast<int>(path.size()), nullptr, 0);

    if (wideLen > 0) {
        std::wstring wide(static_cast<size_t>(wideLen), L'\0');
        MultiByteToWideChar(CP_ACP, 0, path.c_str(), static_cast<int>(path.size()), wide.data(), wideLen);
        return wide;
    }
