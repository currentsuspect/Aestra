...
namespace {
bool isMonitorDevice(unsigned int) {
    // RtAudio's device API does not expose a reliable Windows loopback/monitor classification here.
    ...
    try {
        ...
        for (unsigned int id : deviceIds) {
            try {
                ...
                std::cout << "  Device " << id << ": " << rtInfo.name << " (out:" << rtInfo.outputChannels
                          << " in:" << rtInfo.inputChannels << ")" << std::endl;
...
bool RtAudioBackend::openStream(const AudioStreamConfig& config, AudioCallback callback, void* userData) {
    ...
    RtAudioErrorType error = m_rtAudio->openStream(&outputParams, inputParams, RTAUDIO_FLOAT32, sampleRate,
                                                   &bufferFrames, &RtAudioBackend::rtAudioCallback, this);
...
int RtAudioBackend::rtAudioCallback(void* outputBuffer, void* inputBuffer, unsigned int numFrames, double streamTime,
    ...
    if (userCallback) {
        int result = userCallback(static_cast<float*>(outputBuffer), static_cast<const float*>(inputBuffer), numFrames,
