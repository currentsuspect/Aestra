...
bool IsValidCLSIDFormat(const std::string& clsid) {
    ...
    if (clsid[0] != '{' || clsid[37] != '}')
        return false;
    if (clsid[9] != '-' || clsid[14] != '-' || clsid[19] != '-' || clsid[24] != '-')
        return false;
...
std::vector<ASIODriverInfo> ASIODriverScanner::scanRegistry(const std::string& registryPath) {
    ...
    LONG result = RegOpenKeyExA(HKEY_LOCAL_MACHINE, registryPath.c_str(), 0, KEY_READ, &hKeyASIO);
    ...
    while (RegEnumKeyExA(hKeyASIO, index, driverName, &nameSize, nullptr, nullptr, nullptr, nullptr) == ERROR_SUCCESS) {
...
std::string ASIODriverScanner::getAvailabilityMessage() {
    ...
    if (drivers.empty()) {
        return "No ASIO drivers detected. AESTRA uses WASAPI for professional low-latency audio.";
    }
