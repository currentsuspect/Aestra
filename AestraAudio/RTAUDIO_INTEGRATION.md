...
AestraAudio uses **RtAudio v6.0.1** (MIT License) as its cross-platform audio backend.
...
1. Extract the ASIO SDK zip somewhere on your machine. The project will look for the SDK in a few places.
    - Recommended locations:
       - Inside the repo (recommended for local builds): `AestraAudio/External/ASIOSDK` — convenient for local development but do NOT commit the vendor files to Git; add them to `.gitignore`.
...
3. If RtAudio is compiled with ASIO support, `AestraASIOBackend` will detect that at runtime and request ASIO. If not, the backend will fall back to the default Windows API (WASAPI/DirectSound) and log that ASIO is not available.
...
RtAudio is licensed under the MIT License, which is compatible with Aestra's MIT license.
...
- [RtAudio Documentation](https://github.com/thestk/rtaudio)
