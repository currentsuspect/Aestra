 ...
 * Aestra-Internals: aestra-docs/audio-research-bench.md): Sinc64Turbo reaches ~88 dB single-tone
...
public:
     ...
     */
    void setQuality(ClipResamplingQuality quality) noexcept { m_quality = quality; }
     ...
     */
    inline float getSample(const float* data, int64_t numFrames, uint32_t numChannels, double position,
     ...
     */
    inline void getSampleStereo(const float* data, int64_t numFrames, double position, float& outL,
...
private:
    ...
    inline float sampleLinear(const float* data, int64_t numFrames, uint32_t numChannels, double position,
                              uint32_t channel) const noexcept {
    ...
    inline void sampleLinearStereo(const float* data, int64_t numFrames, double position, float& outL,
                                   float& outR) const noexcept {
        int64_t idx0 = static_cast<int64_t>(position);
        int64_t idx1 = idx0 + 1;
    ...
    inline float sampleCubic(const float* data, int64_t numFrames, uint32_t numChannels, double position,
                             uint32_t channel) const noexcept {
    ...
    inline void sampleCubicStereo(const float* data, int64_t numFrames, double position, float& outL,
                                  float& outR) const noexcept {
    ...
    inline float sampleSinc32(const float* data, int64_t numFrames, uint32_t numChannels, double position,
                              uint32_t channel) const noexcept {
        float outL, outR;
        if (numChannels >= 2) {
            Interpolators::Sinc32Turbo::interpolate(data, numFrames, position, outL, outR);
    ...
    inline float sampleSinc64(const float* data, int64_t numFrames, uint32_t numChannels, double position,
                              uint32_t channel) const noexcept {
