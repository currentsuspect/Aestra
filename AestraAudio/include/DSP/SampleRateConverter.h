...
#if defined(_MSC_VER)
#include <intrin.h>
#define AESTRA_HAS_SSE 1
#if defined(__AVX__) || defined(__AVX2__)
#define AESTRA_HAS_AVX 1
#endif
...
public:
    ...
    SampleRateConverter(SampleRateConverter&& other) noexcept
        : m_srcRate(other.m_srcRate),
          ...
          m_filterBank(other.m_filterBank), // Assuming trivially copyable/movable struct
          m_history(other.m_history), // Assuming trivially copyable/movable struct
          ...
          m_localFilterBank(other.m_localFilterBank), // Assuming trivially copyable/movable struct
    ...
    SampleRateConverter& operator=(SampleRateConverter&& other) noexcept {
        if (this != &other) {
            ...
            m_ratio = other.m_ratio;
            m_srcPosition = other.m_srcPosition;
            m_filterBank = other.m_filterBank;
            ...
            m_nextOutputSrcPos = other.m_nextOutputSrcPos;
            m_sharedFilterBank = std::move(other.m_sharedFilterBank);
            m_localFilterBank = other.m_localFilterBank;
     ...
     */
    void configure(uint32_t srcRate, uint32_t dstRate, uint32_t channels, SRCQuality quality);
     ...
     */
    uint32_t process(const float* input, uint32_t inputFrames, float* output, uint32_t maxOutputFrames) noexcept;
...
private:
    ...
    float interpolateSample(uint32_t channel, uint32_t phaseIndex, int32_t centerPos) const noexcept;
 ...
 */
inline uint32_t estimateOutputFrames(uint32_t inputFrames, uint32_t srcRate, uint32_t dstRate,
