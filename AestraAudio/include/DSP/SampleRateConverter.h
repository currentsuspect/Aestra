...
public:
    ...
    // Must define custom move semantics because atomics are non-movable
    SampleRateConverter(SampleRateConverter&& other) noexcept
        ...
        m_simdEnabled.store(other.m_simdEnabled.load(std::memory_order_relaxed), std::memory_order_relaxed);
    }
