    ...
    public:
        ...
        void addChannel(const std::string& name = "") {
            auto channel = std::make_unique<MixerChannel>(
                name.empty() ? "Track " + std::to_string(m_channels.size() + 1) : name,
        ...
        const PlaylistModel& getPlaylistModel() const {
            return m_playlistModel;
        }
        ...
        void setMeterSnapshots(std::shared_ptr<MeterSnapshotBuffer> snapshots) {
            m_meterSnapshots = snapshots;
        }
        
        /**
         * @brief Get meter snapshots
         */
        std::shared_ptr<MeterSnapshotBuffer> getMeterSnapshots() const {
            return m_meterSnapshots;
        }
        
        /**
         * @brief Get channel slot map
         */
        std::shared_ptr<ChannelSlotMap> getChannelSlotMapShared() const {
            return m_channelSlotMap;
        }
