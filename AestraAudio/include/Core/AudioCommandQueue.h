 ...
 *
 * Uses the existing lock-free ring buffer from AestraCore. Capacity is fixed to
...
public:
    ...
    bool push(const AudioQueueCommand& cmd) {
        ...
        while (depth > prev &&
               !m_maxDepth.compare_exchange_weak(prev, depth, std::memory_order_relaxed, std::memory_order_relaxed)) {}
        return true;
    }
    ...
    uint32_t approxDepth() const noexcept { return static_cast<uint32_t>(m_queue.size()); }

    uint32_t maxDepth() const noexcept { return m_maxDepth.load(std::memory_order_relaxed); }
