...
struct AudioTelemetry {
     ...
     */
    bool isInRecoveryMode() const noexcept { return inRecoveryMode.load(std::memory_order_acquire); }
     ...
     */
    uint32_t getThreadPriorityStatus() const noexcept { return threadPriorityStatus.load(std::memory_order_relaxed); }
    ...
    uint64_t getRecoveryModeActivations() const noexcept {
        return recoveryModeActivations.load(std::memory_order_relaxed);
    }
};
