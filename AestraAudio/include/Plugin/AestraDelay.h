...
public:
    ...
    static float noteDivisionParamFromIndex(int index) {
        return static_cast<float>(std::clamp(index, 0, 12)) / 12.0f;
    }

    static float noteDivisionMultiplier(int index) {
        switch (std::clamp(index, 0, 12)) {
        case kDiv1_1: return 4.0f;
        case kDiv1_2: return 2.0f;
        case kDiv1_4: return 1.0f;
        case kDiv1_16: return 0.25f;
        case kDiv1_8: return 0.5f;
        case kDiv1_32: return 0.125f;
        case kDiv1_2D: return 3.0f;
        case kDiv1_4D: return 1.5f;
        case kDiv1_8D: return 0.75f;
        case kDiv1_16D: return 0.375f;
        case kDiv1_4T: return 2.0f / 3.0f;
        case kDiv1_8T: return 1.0f / 3.0f;
        case kDiv1_16T: return 1.0f / 6.0f;
        default: return 0.5f;
        }
    }
    ...
    static const char* noteDivisionLabel(int index) {
        switch (std::clamp(index, 0, 12)) {
        case kDiv1_1: return "1/1";
        case kDiv1_2: return "1/2";
        case kDiv1_4: return "1/4";
        case kDiv1_16: return "1/16";
        case kDiv1_8: return "1/8";
        case kDiv1_32: return "1/32";
        case kDiv1_2D: return "1/2D";
        case kDiv1_4D: return "1/4D";
        case kDiv1_8D: return "1/8D";
        case kDiv1_16D: return "1/16D";
        case kDiv1_4T: return "1/4T";
        case kDiv1_8T: return "1/8T";
        case kDiv1_16T: return "1/16T";
        default: return "1/8";
        }
    }

    std::vector<uint8_t> saveState() const override {
        struct Blob { uint32_t magic = kStateMagicV2; uint32_t version = 2; float params[kParamCount]; } blob;
    ...
    bool loadState(const std::vector<uint8_t>& state) override {
        ...
        if (header->magic == kStateMagicV2) {
            struct StateBlobV2 { uint32_t magic; uint32_t version; float params[kParamCount]; };
            if (state.size() < sizeof(StateBlobV2)) return false;
            const auto* blob = reinterpret_cast<const StateBlobV2*>(state.data());
            for (uint32_t i = 0; i < kParamCount; ++i) setParameter(i, blob->params[i]);
            snapSmoothedParamsToTargets();
            return true;
        }
        if (header->magic == kStateMagicV1) {
            struct StateBlobV1 { uint32_t magic; uint32_t version; float params[8]; };
            if (state.size() < sizeof(StateBlobV1)) return false;
            const auto* blob = reinterpret_cast<const StateBlobV1*>(state.data());
            for (uint32_t i = 0; i < 8; ++i) setParameter(i, blob->params[i]);
            snapSmoothedParamsToTargets();
            return true;
        }
        return false;
    ...
    uint32_t getTailSamples() const override {
        const float feedback = std::clamp(m_feedbackSmoothed * 0.95f, 0.0f, 0.95f);
        if (feedback <= 0.0001f) return 0;
        const float repeatsTo60dB = -60.0f / (20.0f * std::log10(feedback + 1.0e-6f));
        const float delaySeconds = (getParameter(kSyncMode) > 0.5f)
            ? getSyncedDelaySeconds()
            : (0.01f + m_timeSmoothed * 1.99f);
